...
class Admin:
    def __init__(self, root):
        ...
        lbl_title=Label(self.root,text="CITNC TRANSPORTATION ADMIN PORTAL",font=('Times new roman',30,'bold'),fg='darkblue',bg='white')
        ...
        button_frame.place(x=1300,y=20,width=170,height=210)

        btn_add=Button(button_frame,text="Save",command=self.add_data,font=('arial',14,'bold'),width=13,bg='lawn green',fg='black')
        btn_add.grid(row=0,column=0,padx=1,pady=5)

        btn_upt=Button(button_frame,text="Update",command=self.update_data,font=('arial',14,'bold'),width=13,bg='lawn green',fg='black')
        btn_upt.grid(row=1,column=0,padx=1,pady=5)

        btn_dlt=Button(button_frame,text="Delete",command=self.delete_data,font=('arial',14,'bold'),width=13,bg='lawn green',fg='black')
        btn_dlt.grid(row=2,column=0,padx=1,pady=5)
        ...
        #down frame
        down_frame=LabelFrame(main_frame,bd=2,relief=RIDGE,bg='black',text='Student/Faculty Information Table',font=('Times new roman',11,'bold'),fg='red')
    ...
    def update_data(self):
        if self.var_pos.get()=="" or self.var_id.get()=="":
            ...
            try:
                update=messagebox.askyesno('Update','Are you sure to update this the data ?')
                if update>0:
                    conn=mysql.connector.connect(host='localhost', user='root',password='system123@',database='mydata')
                    my_cursor=conn.cursor()
                    my_cursor.execute('update transport set Position=%s, Name=%s, Phone=%s, Gender=%s, PickP=%s, DropP=%s, PickR=%s, DropR=%s where Id=%s',(
                                                                                                                                                            self.var_pos.get(),
                                                                                                                                                            self.var_name.get(),
                                                                                                                                                            self.var_phno.get(), 
                                                                                                                                                            self.var_gdr.get(),
    ...
    def delete_data(self):
        if self.var_id.get()=="":
            ...
            try:
                Delete=messagebox.askyesno('Delete',"Are you sure to delete this person's data",parent=self.root)
                if Delete>0:
                    conn=mysql.connector.connect(host='localhost', user='root',password='system123@',database='mydata')
                    my_cursor=conn.cursor()
                    sql='delete from transport where Id=%s'
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo('Delete','Person successfully Deleted!',parent=self.root)
            except Exception as es:
                messagebox.showerror('Error',f'Due to:{str(es)}',parent=self.root)
